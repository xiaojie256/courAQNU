    ...
    try:
        ...
        if mode == 'network':
            try:
                ...
                # 🛡️ 【全量动态立体打捞引擎】精确拦截并兼容 data['stdPersonId'] 分支语法
                std_person_id = None
                ...
                # 策略 A：地毯式匹配 HTML 源码中所有的可能 ID 声明（完美捕获 JS 变量赋值）
                ...
                # 策略 B：深度检索 course_data 响应对象的显式元数据键值（隔离过滤 generic "id"）
                if not std_person_id and isinstance(course_data, dict):
                    for key in ["studentId", "stdPersonId", "personId"]:
                ...
                lessons = schedule_data.get("result", {}).get("lessonList", [])
                schedules = schedule_data.get("result", {}).get("scheduleList", [])

                # 🛡️ 拦截保护：若教务网返回数据为空，判定为 ID 错位。拒绝写入缓存，降级渲染历史残像
                if not lessons and not schedules:
                    ...
                    return jsonify({"code": 500, "msg": "防御层拦截：教务网返回空课表，拒绝持久化污染。"}), 500
